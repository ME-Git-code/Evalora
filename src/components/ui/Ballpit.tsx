...
class X {
    ...
    resize() {
        ...
        if (this.#config.size instanceof Object) {
            ...
            w = (this.canvas.parentNode as HTMLElement).offsetWidth || window.innerWidth;
            h = (this.canvas.parentNode as HTMLElement).offsetHeight || 500;
        } else {
            w = window.innerWidth;
    ...
    setColors(colors: number[]) {
        if (Array.isArray(colors) && colors.length > 1) {
            ...
            for (let idx = 0; idx < this.count; idx++) {
                const ratio = idx / (this.count || 1);
                const scaled = ratio * (colors.length - 1);
                const cIdx = Math.floor(scaled);
                const start = colorObjects[cIdx];
                const end = colorObjects[Math.min(cIdx + 1, colors.length - 1)];
                const alpha = scaled - cIdx;
                const outColor = new Color().lerpColors(start, end, alpha);
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
    ...
    function initialize(cfg: any) {
        if (!threeInstance.renderer) return;
        if (spheres) {
            threeInstance.clear();
